...
exports.login = (req, res, next) => {
    ...
    User.findOne({ phone: phone })
        ...
        .then(isEqual => {
            ...
            const responseObj = {
                name: loadedUser.name, phone: loadedUser.phone, email: loadedUser.email,
                pan: loadedUser.pan, gender: loadedUser.gender, role: loadedUser.role,
                isPhoneVerified: loadedUser.isPhoneVerified, alternatePhone: loadedUser.alternatePhone
            };

            res.status(200).json({ token: token, user: responseObj });
        })
        .catch(err => {
            if (!err.statusCode) {
                err.statusCode = 500;
                    ...
                    bcrypt.hash(password, 12)
                        .then(hashedPassword => {
                            User.updateOne({ phone }, { $set: { password: hashedPassword } })
                                ...
                                .catch(err => {
                                    const error = new Error('Error with changing password');
                                    error.statusCode = 401;
                                    throw error;
                                })
                        })
